...
include ReportsHelper
  ...
  if @tab_id!=0 && @status!='All' && @application_status!='All' && params[:start_date] && params[:end_date]
     ...
     @business = BusinessVerification.joins(:business).where('business_verifications.tab_id = ? and business_verifications.created_at >= ? and business_verifications.created_at <= ? and businesses.status = ? and businesses.application_status = ?', @tab_id, @start_date, @end_date, @status, @application_status)
     @work_detail = ClientVerification.joins(:co_applicant_business).where('client_verifications.tab_id = ? and client_verifications.created_at >= ? and client_verifications.created_at <= ? and co_applicant_businesses.status = ? and co_applicant_businesses.application_status = ? ', @tab_id, @start_date, @end_date, @status,@application_status)
